...
# Exercise using an example of the entire play of shakespeare's Romeo and Juliet

```{r}
RJ <- readLines("http://shakespeare.mit.edu/romeo_juliet/full.html")  
```

> side note: You will need some data cleaning to have the text script only. However, we won't worry about it for now. 

Our job is to create a single list where each component includes a single act. Then count how many times the word "Romeo" and "Juliet" appear in each act. Also calculate the number of lines in each act. How should we proceed?
